...
D :   DELETE FROM S1 FIN { printf("Suppression de toutes les lignes de la table \n");};
    ...
    | SELECT AVG PARAO S1 PARAF FROM S1 WHERE C {printf("SELECTING AVG FROM TABLE\n");};
    | SELECT SUM PARAO S1 PARAF FROM S1 WHERE C {printf("SELECTING SUM FROM TABLE\n");};
    | SELECT COUNT PARAO S1 PARAF FROM S1 WHERE C {printf("SELECTING COUNT FROM TABLE\n");};
